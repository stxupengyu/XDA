...
class T5Aug():
    def __init__(self,tokenizer=None,model=None):
        self.tokenizer, self.model= tokenizer, model.cuda()
    ...
    def generate_blanks(self,strings_to_be_generated,
        ...
        **model_kwargs,
        ...
        for batch_idx in range(int(np.ceil(len(strings_to_be_generated)/batch_size))):
            ...
            outputs = [[['British', 'London', '']]]
            # cprint('int(np.ceil(len(strings_to_be_generated)/batch_size))', int(np.ceil(len(strings_to_be_generated)/batch_size)))
            # cprint('input_ids', input_ids)
            # cprint('outputs', outputs)
            # for (b_id,input_id) in enumerate(input_ids):
            #     pred_text=[];result = []
            #     for item in outputs[b_id*num_return_sequences:(b_id+1)*num_return_sequences]:
            #         result.append([]);blanks=[]
            ...
            #                 if token not in [pad_token_id,eos_token_id]:
            #                     output_tokens.append(token)
            pred_blanks.append(outputs)

        print('strings_to_be_generated', strings_to_be_generated)
        pred_texts = None
        return pred_texts,pred_blanks
